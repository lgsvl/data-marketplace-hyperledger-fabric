...
echo -e "\nCreating volume"
if [ "$(kubectl get pvc -n dev | grep shared-pvc | awk '{print $2}')" != "Bound" ]; then
    ...
    kubectl create -f ${KUBECONFIG_FOLDER}/createVolume.yaml -n dev
    sleep 5
 

    if [ "kubectl get pvc -n dev | grep shared-pvc | awk '{print $3}'" != "shared-pv" ]; then
...
kubectl create -f ${KUBECONFIG_FOLDER}/createCouchDBVolume.yaml -n dev
sleep 5